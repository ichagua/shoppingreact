...
const CartItem = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = () => {
    let totalCost = 0;
    cart.forEach((item) => {
      let costo = item.cost.replace(/\D/g, '');
      totalCost += costo * item.quantity;
    });
    return totalCost;
  };
  ...
  const handleDecrement = (item) => {
    ...
    const existingItem = cart.find(obj => obj.name === item.name);
  ...
  const calculateTotalCost = (item) => {
    let costo = item.cost.replace(/\D/g, '');
    let totalCost = costo * item.quantity;
    return totalCost;
  };
